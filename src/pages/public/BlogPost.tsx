  ...
  useEffect(() => {
    ...
    const fetchBlog = async () => {
      try {
        ...
        // Update SEO metadata
        document.title = data.metaTitle || data.title + " | Resource Center";
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-gray-500 font-bold uppercase tracking-widest text-xs">Loading Knowledge...</p>
      </div>
    );
  }
  ...
  if (!blog) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 text-center">
        ...
        <h1 className="text-4xl font-black text-gray-900 mb-2 tracking-tight">Article Not Found</h1>
        <p className="text-gray-500 font-medium mb-8">The blog post you're looking for might have been moved or deleted.</p>
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-gray-50 border-b border-gray-100 py-4 px-6 sticky top-0 z-50 backdrop-blur-md bg-white/80">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3">
             ...
             <button onClick={() => { navigator.clipboard.writeText(shareUrl); alert('Link copied!'); }} className="p-2 text-gray-400 hover:text-indigo-600 transition-colors"><LinkIcon className="w-4 h-4" /></button>
          </div>
        </div>
      </div>
      ...
      <article className="max-w-4xl mx-auto px-6 py-12 md:py-20">
        ...
        <div className="mt-20 pt-10 border-t border-gray-100">
          <div className="flex flex-wrap gap-3">
            <span className="text-sm font-black text-gray-400 uppercase tracking-widest self-center mr-2">Tags:</span>
            {blog.tags.map(tag => (
        ...
        <div className="mt-12 p-8 bg-indigo-600 rounded-[2rem] text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl shadow-indigo-100">
           <div className="text-center md:text-left">
              <h3 className="text-xl font-black mb-1 italic">Found this helpful?</h3>
              <p className="text-indigo-100 font-medium text-sm">Share this resource with your fellow educators and students.</p>
           </div>
           <div className="flex items-center gap-4">
              ...
              <button onClick={() => { navigator.clipboard.writeText(shareUrl); alert('Link copied!'); }} className="px-6 py-4 bg-white text-indigo-600 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-gray-50 transition-all flex items-center gap-2">
      ...
      {relatedPosts.length > 0 && (
        <section className="bg-gray-50 py-20 px-6">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl font-black text-gray-900 mb-10 tracking-tight italic">Recommended Reading</h2>
      ...
      <div className="py-20 text-center">
         <Link to="/blog" className="inline-flex items-center gap-2 text-indigo-600 font-black uppercase tracking-widest hover:gap-4 transition-all">
            Explore All Resources <ChevronRight className="w-5 h-5" />
         </Link>
      </div>
    </div>
  );
